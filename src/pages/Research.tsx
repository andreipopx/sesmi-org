    ...
    <Layout>
      <section className="max-w-7xl mx-auto px-6 pt-12 pb-6">
        ...
        <AnimatedEntry delay={0.25}>
          <div className="border-l-[3px] border-warm bg-bg2 p-5 mb-8 max-w-3xl">
          ...
          <div className="bg-ink text-sesmi-white grid grid-cols-1 lg:grid-cols-2">
            <div className="p-8 md:p-12 lg:border-r border-sesmi-white/10">
              <div className="flex gap-3 mb-6 flex-wrap">
                <span className="font-haas text-[0.55rem] uppercase tracking-[0.15em] text-[#6FCF97] flex items-center gap-1.5">
              ...
              <p className="font-editorial italic text-sesmi-white/50 text-lg mb-4">
                {t.research.featured.subtitle}
              </p>
              <p className="text-sesmi-white/60 text-sm leading-relaxed mb-6">
                {t.research.featured.desc}
              </p>
              <p className="font-haas text-[0.5rem] uppercase tracking-[0.12em] text-sesmi-white/40 mb-8">
                {t.research.featured.methodology}
              </p>
              <Link
                to="/research/meta-datacenter"
                className="border border-sesmi-white/40 text-sesmi-white font-haas text-[0.6rem] uppercase tracking-[0.12em] px-5 py-2.5 hover:bg-sesmi-white/10 transition-colors interactive inline-block"
              >
                ...
                {t.research.featured.chapters.map((ch) => (
                  <div key={ch.n} className="flex items-center justify-between border-b border-sesmi-white/10 pb-3">
                    <div className="flex items-center gap-4">
                      <span className="font-haas text-sm text-sesmi-white/30">{ch.n}</span>
                      <span className="text-sm text-sesmi-white/80">{ch.title}</span>
                    </div>
                    <span className={`font-haas text-[0.5rem] uppercase tracking-[0.12em] ${
      ...
      <section className="max-w-7xl mx-auto px-6 py-8">
        <AnimatedEntry>
          <div className="card-hover-line bg-bg2 p-8 mb-4">
            ...
            <h3 className="font-editorial text-2xl font-bold text-ink mb-2">{t.research.next.title}</h3>
            <p className="font-editorial italic text-sesmi-muted text-sm mb-3">{t.research.next.subtitle}</p>
          </div>
        </AnimatedEntry>
      </section>
